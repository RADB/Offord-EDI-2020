...
var HandshakeProtocol = /** @class */ (function () {
    ...
    HandshakeProtocol.prototype.parseHandshakeResponse = function (data) {
        ...
        if (data instanceof ArrayBuffer) {
            ...
            var separatorIndex = binaryDaEDI.indexOf(TextMessageFormat_1.TextMessageFormat.RecordSeparatorCode);
            if (separatorIndex === -1) {
                throw new Error("Message is incomplete.");
            }
            ...
            messageData = String.fromCharCode.apply(null, binaryDaEDI.slice(0, responseLength));
            remainingData = (binaryDaEDI.byteLength > responseLength) ? binaryDaEDI.slice(responseLength).buffer : null;
        }
        else {
            var textData = data;
            var separatorIndex = textDaEDI.indexOf(TextMessageFormat_1.TextMessageFormat.RecordSeparator);
            if (separatorIndex === -1) {
                throw new Error("Message is incomplete.");
            }
            // content before separator is handshake response
            // optional content after is additional messages
            var responseLength = separatorIndex + 1;
            messageData = textDaEDI.substring(0, responseLength);
            remainingData = (textDaEDI.length > responseLength) ? textDaEDI.substring(responseLength) : null;
        }
...
function getDataDetail(data, includeContent) {
    var length = null;
    if (data instanceof ArrayBuffer) {
        length = "Binary data of length " + daEDI.byteLength;
        if (includeContent) {
            length += ". Content: '" + formatArrayBuffer(data) + "'";
        }
    }
    else if (typeof data === "string") {
        length = "String data of length " + daEDI.length;
        if (includeContent) {
            length += ". Content: '" + data + "'.";
        }
    }
        ...
        return tslib_1.__generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 1:
                    ...
                    logger.log(ILogger.LogLevel.Trace, "(" + transportName + " transport) sending daEDI. " + getDataDetail(content, logMessageContent) + ".");
...
var WebSocketTransport = /** @class */ (function () {
    ...
    WebSocketTransport.prototype.send = function (data) {
        if (this.webSocket && this.webSocket.readyState === WebSocket.OPEN) {
            this.logger.log(ILogger.LogLevel.Trace, "(WebSockets transport) sending daEDI. " + Utils.getDataDetail(data, this.logMessageContent) + ".");
